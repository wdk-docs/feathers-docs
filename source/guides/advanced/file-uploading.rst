...
4. At the moment there is no third party storage service involved, but
   this will change in the near future, so it has to be prepared.
...
Fortunately, there exists a file storage service:
...
problems which we’ll discuss below.

Basic upload with feathers-blob and feathers-client
---------------------------------------------------

For the sake of simplicity, we will be working over a very basic
feathers server, with just the upload service.
...
It doesn’t feels right because it is not. Let’s imagine what would
...
The solution would be to modify the service, adding support for
...
the very early days of the web? maybe since Netscape Navigator?
...
a custom middleware to handle that.
...
The solution is to switch ``feathers-client`` from REST to
...
All the code is available via github here:
https://github.com/CianCoders/feathers-example-fileupload
