...
1. Obtain the JWT by either going through the oAuth or local
   authentication flow
2. Set the JWT in the cookie (since the browser will send it with every
   request)
3. Before any middleware that needs to be protected, add the
...
In order for the browser to send the JWT with every request, cookies
have to be enabled in the authentication configuration.
...
The :doc:`../api/authentication/jwt`
...
Now we can protect any Express route by first adding
``cookieParser(), authenticate('jwt')`` to the chain.

.. note:: Only register the cookie parser middleware before routes
   that actually need to be protected by the JWT in the cookie in order
   to prevent CSRF security issues.
...
-  Set the view engine to EJS (the default folder for views in Express
   is ``views/`` in the root of the project)
...
-  Register a ``/logout`` route that deletes the cookie and redirect
   back to the login page

..

.. note:: We could also generate the middleware using
   ``feathers generate middleware`` but since they are all fairly short
   we can keep it in the same file for now.
...
.. note:: ``npm ls @feathersjs/authentication-jwt`` has to show that
   version 2.0.0 or later is installed.
...
similar to the :doc:`chat/frontend`:
...
will log us out and redirect to the login page.
