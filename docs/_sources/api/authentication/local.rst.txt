...
.. important:: When setting the ``usernameField`` to ``username`` in
   the :doc:`../configuration` the value *must* be
...
.. important::
   **@feathersjs/authentication-local** does not allow to store clear text passwords.
   This means the ``hashPassword`` hook **must** be used when using the :ref:`local_verifier`.

Available options are

-  ``passwordField`` (default: ``'password'``) - key name of password
   field to look on context.data
...
This is the verification class that does the actual username and
password verification by looking up the entity (normally a ``user``) on
a given service by the ``usernameField`` and compares the hashed
password using bcrypt. It has the following methods that can all be
overridden. All methods return a promise except ``verify``, which has
the exact same signature as
`passport-local <https://github.com/jaredhanson/passport-local>`_.
...
Authenticating using a local strategy via sockets is done by emitting
...
.. |npm version| image:: https://img.shields.io/npm/v/@feathersjs/authentication-local.png?style=flat-square
   :target: https://www.npmjs.com/package/@feathersjs/authentication-local
.. |Changelog| image:: https://img.shields.io/badge/changelog-.md-blue.png?style=flat-square
   :target: https://github.com/feathersjs/feathers/blob/master/packages/authentication-local/CHANGELOG.md
