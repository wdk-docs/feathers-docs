...
export <https://nodejs.org/api/modules.html>`_ for that file:
...
``app.configure`` them in ``app.js``

.. note:: Keep in mind that the order in which configure functions
...
We already saw in the :doc:`./hooks` how we can create a
...
This is also the pattern the hook generator uses but in its own file
like ``hooks/set-timestamp.js`` which could look like this:
...
.. note:: We are using an options object here which allows us to more
   easily add new options than function parameters.
